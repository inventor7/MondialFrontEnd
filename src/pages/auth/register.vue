            ...
            <div class=" flex flex-col justify-start items-center w-full h-fit space-y-10  ">
                ...
                <form class="flex flex-col justify-center items-center w-full h-full space-y-6" action="" method="post"
                    ...
                    <div class="space-y-2 w-full ">
                        <Input class=" h-10 " v-model="form.name" type="text" autocomplete="name" required
                            placeholder="Name..." />
                    ...
                    <div class="space-y-2 w-full ">
                        <Input class=" h-10 " v-model="form.password" autocomplete="new-password" required type="password"
                    ...
                    <div class="space-y-2 w-full ">
                        <Input class=" h-10 " v-model="form.password_confirmation" autocomplete="new-password" required
                            type="password" placeholder="confirme password..." />
                        <span class="text-red-600 w-full text-start  text-sm" v-if="Object.keys(error).length !== 0">{{
                    ...
                    <router-link to="/auth" class=" w-full ">
...
let isConfirmationPasswordValid = computed(() => {
    return (form.value.password_confirmation.length >= 6 && form.value.password === form.value.password_confirmation)
})
...
const handleLogin = async () => {
    if (isEmailValid.value && isPasswordValid.value) {
        ...
        try {
            res.value = await axios.post("/register", form.value);
            console.log(res)
        } catch (err) {
            error.value = err
